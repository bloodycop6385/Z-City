...
local font = function()
    -- hg_coolvetica:GetBool() and "Coolvetica" or "Bahnschrift"
...
function hg.GetConVarType(convar)
    ...
    if (stringv == '0' and not boolVal) or (stringv == '1' and boolVal) then return 'bool' end
    if tonumber(stringv) and math.floor(stringv) == floatVal then if intVal == floatVal then return "int" end end
    return "string"
end
...
function hg.CreateButton(buttonData, convarName, ParentPanel, yPos)
    ...
    if convarType == 'bool' then
        ...
        function toggle:Paint(w, h)
            if animProgress ~= targetProgress then animProgress = Lerp(FrameTime() * 8, animProgress, targetProgress) end
            local bgColor = Color(Lerp(animProgress, 180, 80), Lerp(animProgress, 30, 120), Lerp(animProgress, 30, 50))
    ...
    elseif convarType == 'int' then
        ...
        function slider:OnValueChanged(val)
            if convar then SetConVarValue(convar, decimals > 0 and math.Round(val, decimals) or math.Round(val)) end
        end
        ...
        slider.Think = function() if convar then valueLabel:SetText(convar:GetInt()) end end
    elseif convarType == 'string' then
        ...
        function textEntry:OnValueChange(val)
            if convar then SetConVarValue(convar, val) end
        end
    end
    return pppanel
end
